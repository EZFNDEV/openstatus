  ...
  function groupByDay(
    ...
    const grouped = [...maintenances, ...statusReports].reduce(
      (acc, element) => {
        ...
        if (isMaintenance) {
          const date = new Date(typeof element.from === 'string' ? element.from : element.from.toDateString()).getTime();
        ...
        if (isStatusReport) {
          ...
          const date = (
            firstUpdate
              ? new Date(typeof firstUpdate?.date === 'string' ? firstUpdate?.date : firstUpdate?.date)
